...
describe('CheckoutService', () => {
  ...
  describe('reserve', () => {
    ...
    it('should throw error if active reservation exists', async () => {
      ...
      await expect(service.reserve(userId, items)).rejects.toThrow('User already has an active reservation');
    });
  ...
  describe('confirm', () => {
      ...
      const mockReservation = {
          ...
          total_cents: 2000,
          currency: 'MXN',
          items: [{ product_variant_id: 1, quantity: 2, unit_price_cents: 1000, total_price_cents: 2000 }],
      ...
      it('should throw error if reservation not found', async () => {
          (redis.get as jest.Mock).mockResolvedValue(null);
          await expect(service.confirm(userId, reservationId, paymentIntentId)).rejects.toThrow('Reservation not found');
      });
  });
});

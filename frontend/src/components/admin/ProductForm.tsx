...
export function ProductForm({ initialData, onSubmit, onCancel, isLoading, categories }: ProductFormProps) {
  const [formData, setFormData] = useState({
    name: initialData?.name || '',
    description: initialData?.description || '',
    price: initialData?.price.toString() || '',
    category: typeof initialData?.category === 'object' ? (initialData.category as any).id.toString() : (initialData?.category || ''),
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label htmlFor="images" className="block text-sm font-bold text-neutral-700 mb-2">Imágenes</label>
        
        {/* Images List */}
        {images.length > 0 && (
            <div className="mb-4">
                <p className="text-xs text-gray-500 mb-2">Ordena las imágenes arrastrando o usando los botones:</p>
                <div className="flex gap-2 flex-wrap">
                    {images.map((img, index) => (
                        <div key={index} className="relative w-32 h-32 group border rounded-lg overflow-hidden bg-gray-50">
                            <img 
                                src={img.preview} 
                                alt={`Product ${index}`} 
        ...
        <p className="text-xs text-gray-500 mt-1">Puedes seleccionar múltiples imágenes. Se añadirán al final.</p>
      </div>
